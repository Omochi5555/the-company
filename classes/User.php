<?php
// To inherit Database class...
require "Database.php";
class User extends Database // Now, 'conn' is available
{
    ...
    public function saveUser($first_name, $last_name, $username, $password)
    {
        ...
        if ($this->conn->query($sql)) { //execute SQL statement (insert into table)
            //go to login page
    ...
    public function login($username, $password)
    {
        ...
        if ($result->num_rows == 1) { // if user is found
            ...
            if (password_verify($password, $user_data["password"])) {
                //if password correct, create the session
                session_start();
